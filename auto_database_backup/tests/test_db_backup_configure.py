...
class TestDbBackupConfigure(TransactionCase):
    """Tests for the db.backup.configure model (Phase 1 refactor)."""
    ...
    def test_05_token_generated_flags(self):
        """The token-generated boolean flag reflects the stored tokens."""
        config = self._create_config()
    ...
    def test_06_redirect_uri_compute(self):
        """The redirect URI is computed from the instance base URL."""
        config = self._create_config()
        self.assertTrue(
    ...
    def test_11_oauth_redirect_blocks_external_url(self):
        ...
        self.assertEqual(
            GoogleDriveAuth._safe_redirect_url('https://evil.example.com/x',
                                               base_url), '/web')
        # A blank url_return is refused too.
        self.assertEqual(
            GoogleDriveAuth._safe_redirect_url(False, base_url), '/web')
        # A local url_return is preserved.
        local = base_url + '/web#id=1&model=db.backup.configure'
        self.assertEqual(
            GoogleDriveAuth._safe_redirect_url(local, base_url), local)
