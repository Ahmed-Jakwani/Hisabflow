...
PYTHON_ENV = sys.executable or "/usr/bin/python3"
LOG_FILE_PATH = "/var/log/odoo19/backup_cron.log"
...
class Cronjob:
    ...
    def set_now_time_for_cron(self, user_timezone):
        """ 
            Set cron to run 5 minutes from now
        """
        _logger.info(self.frequency_cycle)
        ...
        # Extract components from target time
        ...
        if self.frequency_cycle == 'half_day':
            # Run every 12 hours starting from target_time
            second_hour = (target_hour + 12) % 24
            self.job.hour.on(target_hour, second_hour)
            self.job.minute.on(target_minute)
            self.job.dow.on()
            self.job.dom.on()
            return True
            ...
            _logger.info("time %r" % target_hour)
            if self.frequency == 1:
                self.job.dow.on(target_dow)
                self.job.hour.on(target_hour)
                self.job.minute.on(target_minute)
                self.job.dom.on()
                return True
            ...
            if self.frequency == 1:
                ...
                self.job.hour.on(target_hour)
                self.job.minute.on(target_minute)
                self.job.month.on(target_month)
                return True
            else:
                return False
    ...
    def update_cronjob_immediate(self, process_id,timezone):
        _logger.info("Running Job In Next 5 Minutes")
        process_id = "--processid "+process_id+" "
        jobs = list(self.list_cronjobs(process_id))
        if len(jobs) == 1:
            _logger.info("%s"%(jobs[0]))
            self.job = jobs[0]
            res = self.set_now_time_for_cron(timezone)
            if res:
                return {
                        'success': True,
                        'msg': self.job
...
def immediate_run_cron(db_name, process_id, frequency, frequency_cycle, timezone='UTC'):
    create_time = datetime.datetime.now().strftime("%m/%d/%Y, %H:%M:%S")
    cj = Cronjob(create_time, frequency, frequency_cycle)
    cj.command = db_name
    uc = cj.update_cronjob_immediate(process_id, timezone)
    if uc['success'] == False:
        return uc
    else:
        wc = cj.write_crontab()
