...
class CustomerPortal(CustomerPortal):
    ...
    @http.route('/my/saas/contract/add/domain', type='json', auth="public",  website=True)
    def add_domain_contract_view(self, contract_id, domain_name,is_ssl,use_seperate_domain):
        contract = request.env['saas.contract'].sudo().browse(int(contract_id))
        if not use_seperate_domain:
            domain_name += "." + contract.server_id.server_domain
        response = request.env['custom.domain'].add_subdomain_call(contract_id, domain_name,is_ssl)
        #_logger.info(f"====add_domain_contract_view===={response}================")
        values={
            'contract': contract,
            }
        res = request.env['ir.ui.view'].sudo()._render_template('odoo_saas_kit.portal_domain_template', values)
        vals = {}
        vals['data'] = res
