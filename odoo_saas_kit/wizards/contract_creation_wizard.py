...
class ContractCreation(models.TransientModel):
    ...
    # , domain="[('saas_plan_id', '=', 1)]"
    # , domain="[('saas_plan_id', '=', self.plan_id.id)]"
    ...
    def invoice_product_id_change(self):
        ...
        if self.plan_id.product_template_ids:
            int_plan_id = int(self.plan_id.id)
            return {
    ...
    def action_create_contract(self):
        for obj in self:
            ...
            if not obj.plan_id.is_multi_server:
                ...
                priority_servers = obj.plan_id.default_saas_servers_ids.sorted(lambda s: s.priority)
