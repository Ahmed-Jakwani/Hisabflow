...
# ==================RENEW MAIL CRON FUNCTION==========================#
    ...
    def send_invoice_mail(self, invoice):
        ...
        subjects = mail_template._render_template(template.subject, 'account.move', [invoice.id]).get(invoice.id)
        render_body = getattr(mail_template, '_render_template_qweb', mail_template._render_template)
        body = render_body(template.body_html, 'account.move', [invoice.id]).get(invoice.id)
        emails_from = mail_template._render_template(template.email_from,'account.move', [invoice.id]).get(invoice.id)
